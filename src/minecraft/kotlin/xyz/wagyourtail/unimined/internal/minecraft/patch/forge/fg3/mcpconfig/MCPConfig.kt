...
class MCPConfig(
    ...
    private val steps: MutableMap<String, ConfigStep> by lazy {
        mutableMapOf<String, ConfigStep>().apply {
            ...
            for (step in configJson.getAsJsonObject("steps").getAsJsonArray(provider.side.name.lowercase())) {
                ...
                val step: ConfigStep =
                    when (type) {
                        "downloadManifest", "downloadJson" -> {
                            NoOpStep(name, prevStep, vars)
                        }

                        "downloadClient" -> {
                            FileProviderStep(name, prevStep, vars) {
                        ...
                        "listLibraries" -> {
                            ListLibrariesStep(name, prevStep, vars)
                        }
                        ...
                        "preProcessJar" -> {
                            FunctionStep(name, prevStep, vars, functions.getValue(type))
                        }
    ...
    private val stepResult =
        defaultedMapOf<String, StepResult> {
            val step = steps.getValue(it)
            step.prev?.let { this.getValue(it) }
            val dir = cacheDir.resolve(it).createDirectories()
            if (step.cacheable && isCached(step, dir)) {
                project.logger.info("[Unimined/MCPConfig] Reusing cached step: ${step.name}")
                return@defaultedMapOf StepResult(it, step.outputFile(dir))
            }
            project.logger.info("[Unimined/MCPConfig] Executing ${step::class.simpleName}: ${step.name}")
            val result: StepResult
            try {
                measureTime {
                    result = step.execute(dir)
                }.also {
    ...
    private fun fingerprint(
        ...
        buildString {
            for ((k, v) in step.variables.entries.sortedBy { it.key }) {
                append(k).append('=').append(hashInput(v.invoke())).append('\n')
            }
            append(step.extraFingerprint(dir))
        }
    ...
    private fun isCached(
        ...
        if (project.unimined.forceReload) return false
        val output = step.outputFile(dir)
        if (!output.exists()) return false
        val fpFile = dir.resolve("${step.name}.fingerprint")
        if (!fpFile.exists()) return false
        return fpFile.readText() == fingerprint(step, dir)
    }
    ...
    private fun writeFingerprint(
        ...
        val fpFile = dir.resolve("${step.name}.fingerprint")
        val tmp = fpFile.resolveSibling("${fpFile.name}.tmp")
        tmp.writeText(fingerprint(step, dir))
        tmp.moveTo(fpFile, StandardCopyOption.REPLACE_EXISTING)
    }

    fun getResultFor(step: String): StepResult = stepResult.getValue(step)
    ...
    interface ConfigStep {
        ...
        /** Whether this step participates in the cross-build output cache. */
        val cacheable: Boolean
            get() = false

        /** The cached output file, or null for steps without a persistent output. */
        fun outputFile(dir: Path): Path = dir.resolve("${name}Output.jar")

        /** Extra fingerprint content (e.g. function args/classpath), empty by default. */
        fun extraFingerprint(dir: Path): String = ""

        fun execute(dir: Path): StepResult
    }

    inner class NoOpStep(
        override val name: String,
        override var prev: String?,
        override val variables: MutableMap<String, () -> String>,
    ) : ConfigStep {
        override fun execute(dir: Path): StepResult = StepResult(name, null)
    }

    inner class FileProviderStep(
        override val name: String,
        override var prev: String?,
        override val variables: MutableMap<String, () -> String>,
        val provider: ConfigStep.(Path) -> Path,
    ) : ConfigStep {
        override fun execute(dir: Path): StepResult = StepResult(name, provider(dir))
    }
    ...
    open inner class FunctionStep(
        ...
        override fun extraFingerprint(dir: Path): String =
            buildString {
                val output = dir.resolve("${name}Output.jar")
                append("args=").append(function.args.mapVariables(output)).append('\n')
                append("jvmargs=").append(function.jvmargs.mapVariables(output)).append('\n')
                append("classpath=").append(function.classpath).append('\n')
                append("mainClass=").append(function.mainClass).append('\n')
                append("javaVersion=").append(function.javaVersion).append('\n')
            }
        ...
        open fun resolveVariable(
            ...
            when (varName) {
                ...
                "log" -> {
                    output.resolveSibling("log.txt").absolutePathString()
                }

                else -> {
                    variables[varName]?.invoke()
                ...
                .javaexec {
                    if (useToolchains) {
                                        ...
                                        .launcherFor {
                                            it.languageVersion.set(
                                                JavaLanguageVersion.of(
                                                    provider.minecraftData.metadata.javaVersion.majorVersion
                                                        .toInt(),
                                                ),
                                            )
                                        }.get()
                        ...
                        if (JavaVersion.current() < (JavaVersion.toVersion(function.javaVersion ?: 8))) {
                            error(
                                "current java version ${JavaVersion.current()} is less than required java version ${function.javaVersion} to run ${function.classpath}",
                            )
                        }
                    }
                        ...
                        JarFile(jar).use { jf ->
                            it.mainClass.set(
                                jf.manifest.mainAttributes.getValue(Attributes.Name.MAIN_CLASS)
                                    ?: error("No Main-Class in manifest of $jar"),
                            )
                        }
                    }
